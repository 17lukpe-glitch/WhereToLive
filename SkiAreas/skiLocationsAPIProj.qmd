...
# Sets working directory to this project's folder
setwd("C:/Users/Luke/OneDrive/Documents/Fireweed Cartography/WhereToLive/SkiAreas")
```
...
areasByYear <- ggplot(byYear) +
    ...
    theme_bw() +
    theme(plot.title = element_text(face = "bold"),
          axis.text.x = element_text(angle = 270, vjust = 0.35, size = 6))
...
skiPoints <- vect(finalSkiData, crs = "+proj=longlat +datum=WGS84", geom = c("long", "lat"))
...
emptDistRaster <- rast(crs = "+proj=longlat +datum=WGS84", 
                   extent = ext(skiPoints), 
                   resolution = 0.1)

# Calculates the distance to any cell in the empty raster from the nearest point in
# skiPoints
distRaster <- distance(emptDistRaster, skiPoints, unit = "km", method = "geo")

# A plot to see just what witch's brew I cooked up
plot(distRaster)
```

Slap my ass and call me a mule, I think that did it. Or at least it did something. My gut is telling me that there is something weird going on with the scale. It seems like shouldn't be so many blueish lines between the points, but I am also hesitant to make any judgments yet because I know this raster has distortions in area and distance, hence why the auras around the Alaskan resorts are so large. I will need to think about this. I may also need to use the extent from my interpolation of rents if I want to use this for a suitability model.

```{r writeFile}

writeRaster(distRaster, "prelimSkiRaster.tif")
```
